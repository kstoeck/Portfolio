...
if (array_length(lines) > 0) {
    var column1 = string_split(lines[1], ","); // e.g. ["0.05", "0"]

    // Safety check: make sure we have at least 2 columns
    if (array_length(column1) >= 2) {
        var val2 = real(column1[1]); // second value

        // Set image_index based on val2
        if (val2 == 8) {
            image_index = 2; // Direct mapping
        } else {
            image_index = 0; // Default to 0 if val2 is out of range
        }
    }
}